...
for b in np.unique(indices):
    if b == 15:
        ...
        print(np.array(phases)[indices == b])
        print(np.min(np.array(phases)[indices == b]))
        print(np.max(np.array(phases)[indices == b]))
        ...
        currentphases = np.array(phases)[indices == b]
        currentimes = np.array(times)[indices == b]
        ...
        plt.hist2d(unc_mean, cor_mean, bins = 100, rasterized=True)
        ...
        plt.tight_layout()
        plt.savefig("non-cor-cor.pgf")
        plt.show()

        plt.figure(figsize = (3, 3))
        plt.hist2d(phs_mean, cor_mean, bins = 100, rasterized=True)
        plt.xlabel("Phase (rads)")
        plt.ylabel("Correlation averaged mean (px)")
        plt.axvline(np.mean(currentphases), color = "red", alpha = 0.5)
        plt.tight_layout()
        plt.savefig("phase-cor.pgf")
        plt.show()

        plt.figure(figsize = (3, 3))
        plt.hist2d(tim_mean, cor_mean, bins = 100, rasterized=True)
        plt.xlabel("Time (s)")
        plt.ylabel("Correlation averaged mean (px)")
        plt.axvline(np.mean(tim_mean), color = "red", alpha = 0.5)
        plt.tight_layout()
        plt.savefig("time-cor.pgf")
        plt.show()
