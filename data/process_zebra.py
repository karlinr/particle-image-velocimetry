...
for g, grouping in enumerate(plist):
    video_array = np.zeros((len(grouping) * 2, tf.imread(f"data/zebrafish/unprocessed/030298.tif").shape[1],
                            tf.imread(f"data/zebrafish/unprocessed/030298.tif").shape[2]))
    f = 0
    for video in os.listdir("zebrafish/unprocessed/"):
        if video.endswith(".tif"):
            current_video = tf.imread(f"data/zebrafish/unprocessed/{video}")
            for frame in grouping:
                ...
                if lower_index <= frame < upper_index:
                    ...
                    f += 2
    tf.imwrite(f"data/zebrafish/processed/{g}.tif", video_array.astype(np.int16))
